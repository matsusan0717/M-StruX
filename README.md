# M-Bare — Blogger テンプレート

M-Bare は、Blogger 向けに設計されたシンプル・軽量なカスタムテンプレートです。一般的な Blogger テーマとは異なり、テーマデザイナーや管理画面のレイアウト編集には対応していません。ガジェットの追加・削除・並び替えはレイアウト画面から行うことができず、カスタマイズはすべて XML（テンプレートHTML）を直接編集することで行います。そのため、ある程度 HTML / CSS / JavaScript の知識があるユーザー向けのテンプレートです。

表示まわりの機能は JavaScript によるクライアントサイドレンダリングで実装されており、記事一覧・個別記事・ラベル・検索・アーカイブなどのページ出し分けをすべてスクリプト側で制御しています。Blogger 標準のページ遷移を排除し、JSON Feed API を使ってコンテンツを動的に取得・描画する SPA（シングルページアプリケーション）型構成を採用しています。これにより、Blogger 標準のウィジェット構造に縛られることなく、自由度の高いレイアウトと高速なページ表示を実現しています。
...
また、コードブロックのコピーボタン、メガメニュー、プロフィールカード、検索・アーカイブページの noindex 制御、構造化データ（JSON-LD）、Twitter Card、Canonical URL 自動設定など、SEO・UX を意識した機能も標準搭載しています。
...
- **コードコピーボタン** — `pre / code` を自動検出してコピー機能を追加
